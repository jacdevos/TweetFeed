...
class TweetRelevanceSorterTests: XCTestCase {
    ...
    func testRankShouldIncreaseWithMoreRetweets() {
        XCTAssertGreaterThan(createTweet(retweets: 100).rank(),createTweet(retweets: 1).rank())
        XCTAssertGreaterThan(createTweet(retweets: 100).rank(),createTweet(retweets: 0).rank())
        XCTAssertLessThan(createTweet(retweets: 1).rank(),createTweet(retweets: 100).rank())
    }
    
    func testRankShouldIncreaseWithMoreFavourites() {
        XCTAssertGreaterThan(createTweet(favourites: 100).rank(),createTweet(favourites: 1).rank())
        XCTAssertGreaterThan(createTweet(favourites: 100).rank(),createTweet(favourites: 0).rank())
        XCTAssertLessThan(createTweet(favourites: 1).rank(),createTweet(favourites: 100).rank())
    }
    ...
    func testRankShouldLowerWithMoreFollowers() {
        ...
        XCTAssertGreaterThan(createTweet("1", retweets: 100,followersCount: 0).rank(),createTweet("2", retweets: 100,followersCount: 1000).rank())
    }
    ...
    func createTweet(id : String = "1", retweets : Int = 0, favourites : Int = 0, followersCount : Int = 0) -> Tweet{
        return Tweet(JSONDictionary: ["retweet_count":retweets, "favorite_count":favourites,"id_str":id, "user":["followers_count":followersCount]])
    }
}
